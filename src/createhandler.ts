...
const createHandlerAsync = (
...
): Function => async (event: APIGatewayEvent, context: LambdaContext): Promise<APIGatewayProxyResult> => {
  ...
  return app.getAppResponse(new AwsLambdaContext(event, context, isColdStart))
